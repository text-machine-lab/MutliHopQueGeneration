...
def main(args):
    ...
    for eval_checkpoint in sorted(eval_checkpoints, reverse=True):
        start = time.time()
        ...
        with tf.io.gfile.GFile(os.path.join(MODEL_DIR, f"eval_{eval_checkpoint}.json"), "w") as f:
        ...
        with tf.io.gfile.GFile(os.path.join(MODEL_DIR, f"eval_metrics.json"), "a") as f:
    ...
    # Write out the supplied test examples to text files
    predict_inputs_path = {task_num: os.path.join(MODEL_DIR, f"predict_inputs_test_{task_num}.txt") for task_num in range(1, 14)}
    predict_outputs_path = {task_num: os.path.join(MODEL_DIR, f"predict_outputs_test_{task_num}.txt") for task_num in range(1, 14)}
    ...
    with tf.io.gfile.GFile(os.path.join(MODEL_DIR, f"test_eval_metrics.json"), "a") as f:
        f.write(f"checkpoint: {eval_checkpoint}" + ', '.join([f"{key}: {metrics[key]:.2f}" for key in sorted(metrics.keys())]) + '\n')
    fin = time.time()
